...
unintentional_injuries <- read.csv("data/ui_admissions_2023.csv")
unintentional_injuries <- unintentional_injuries %>% 
  clean_names()
```
...
```{r}
summary(unintentional_injuries)
str(unintentional_injuries)
filtered_injuries <- unintentional_injuries %>% 
  filter(sex =="Female", age_group == "75plus years", injury_type == "Falls", financial_year == "2022/23", hbr != "S92000003", injury_location == "Home")

falls_admissions_75_summary <- filtered_injuries %>% 
  group_by(hbr) %>% 
  summarise(sum(number_of_admissions))
```
...
join_data <- full_join(NHS_healthboards, falls_admissions_75_summary, join_by(HBCode == hbr))
```
...
```{r}
falls_map <- join_data %>% 
  ggplot(aes(fill = `sum(number_of_admissions)`)) +
  geom_sf() +
  scale_fill_distiller(palette = 15) +
  labs(title = "Number of falls at home in the female 75+ age group across NHS health boards in Scotland")
falls_map
```
...
```{r}
hbdata <- read.csv("data/hb2019_pop_est.csv")

filtered_injuries_per1000 <- hbdata %>% 
  filter(Sex =="Female", Year == "2022", HB != "S92000003") %>% 
  mutate(population = rowSums(across(Age75:Age90plus)))

filtered_injuries_per1000 = filtered_injuries_per1000 %>% 
  select(Sex, Year, HB, population)
#potentially better alternative is to pivot longer 

joined_2022_data <- full_join(falls_admissions_75_summary,filtered_injuries_per1000, join_by(hbr == HB))

joined_2022_data <- joined_2022_data %>% 
  mutate(falls_per_10k = `sum(number_of_admissions)`/(population/10000))

joined_geom_data <- full_join(NHS_healthboards, joined_2022_data, join_by(HBCode == hbr))

joined_geom_plot <- joined_geom_data %>% 
  ggplot(aes(fill=falls_per_10k)) +
  geom_sf() +
  scale_fill_distiller(palette = 1) +
  labs(title = "Number of falls at home in the female 75+ age group per 1000 across NHS health boards in Scotland in 2022") +
    annotation_scale(    
    location = "tl"
  ) +
